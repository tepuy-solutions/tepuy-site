...
const pct = id => num(id) / 100;/* ---------- global state ---------- */
const FREE_LIMIT = 10;
const usesKey    = "pvProUses";
const paidKey    = "pvProPaid";let paid = localStorage.getItem(paidKey) === "yes";
let uses = +localStorage.getItem(usesKey) || 0;let chart;
let csvRows = [];/* ---------- quick upfront panel ---------- */
function calculateBasic() {
  const loanAmt = num("loanAmount");
  const dpPct   = pct("downpayment");
  const costs   = num("buyingCosts");
  const yrs     = num("loanPeriod");
  const rLoan   = pct("loanInterestRate");  const lmiPct = dpPct >= 0.20 ? 0
  ...
  const wkPay  = (((rLoan / 12) * loanAmt *
                  ...
                  (Math.pow(1 + rLoan / 12, yrs * 12) - 1)) * 12 / 52;  $("lmiPercentage").value    = (lmiPct * 100).toFixed(2);
  $("lmiAmount").value        = fmt(lmiAmt);
  $("buyPrice").value         = fmt(price);
  $("totalCashUpfront").value = fmt(cashUp);
  $("weeklyPayment").value    = wkPay.toFixed(2);  return { loanAmt, dpPct, costs, yrs, rLoan, price, cashUp, wkPay };
}/* ---------- full projection ---------- */
function drawProjection(base, showTable) {
  ...
  const sellFlag = $("sellAtRet").checked;  const lastYear = sellFlag ? yrsRet : base.yrs;  let propVal = base.price,
      shares  = base.cashUp,
      owed    = base.loanAmt;  const labels = [], equityArr = [], sharesArr = [];
  csvRows = [["Year","Property","Owed","Equity",
            "OwnCosts","Rent","Interest","Depr","Amort","NetCF", "Shares"]];  for (let y = 0; y <= lastYear; y++) {
    ...
    if (y) owed    = Math.max(0, Math.round(owed * (1 + base.rLoan) - base.wkPay * 52));let equity = propVal - owed;
...
csvRows.push([y, propVal, owed, equity,
          ownCost, rent, interest, depr, amort, netCF, shares]);  }/* ---- chart ---- */
if (chart) chart.destroy();
const ctx = $("investmentChart");
chart = createTepuyStyledChart(ctx, labels, equityArr, sharesArr);  /* ---- table ---- */
  if (showTable) {
const rows = csvRows.slice(1)
  .map(r => <tr>${r.map((c, i) => {     const highlightCols = [3, 10]; // 3 = Equity, 10 = Shares Value     const cls = highlightCols.includes(i) ? ' class="highlight"' : '';     return <td${cls}>${fmt(c)}</td>;   }).join("")}</tr>).join("");$("results").innerHTML =
  <div class="table-container"><table class="results-table">      <thead><tr>        <th>Yr</th><th>Property Value</th><th>Capital Owed</th>        <th class="highlight">Equity</th>        <th>Own Costs</th><th>Rent</th><th>Interest</th>        <th>Depr.</th><th>Amort.</th><th>Net CF</th>        <th class="highlight">Shares Value</th>      </tr></thead>      <tbody>${rows}</tbody></table></div>;  } else {
    $("results").innerHTML = "";
  }
}/* ---------- CSV & Save (Pro only) ---------- */
...
function recommendPro() {
  ...
  overlay.innerHTML =      <div class="overlay-card">       <h2 class="modal-title"> Go Pro with Tepuy</h2>       <p class="modal-subtitle">You've run the calculator <strong>${uses}</strong> times!</p>       <ul class="modal-benefits">         <li> Full investment results for every year</li>         <li> CSV export to Excel</li>         <li> Save & reload custom scenarios</li>         <li> Capital Gains Tax + Sale Cost modeling</li>         <li> NPV & IRR (coming soon)</li>         <li> Indicative tax impact comparisons</li>       </ul>       <button id="goPro" class="btn">Unlock Pro – A$9</button>       <p><a href="#" id="keepFree" class="text-link">No thanks, keep free version</a></p>     </div>;
  document.body.appendChild(overlay);
  document.querySelector("main").classList.add("blur");  $("goPro").onclick    = () => { overlay.remove(); startCheckout(); };
  $("keepFree").onclick = e => {
    e.preventDefault();
...
function calculate() {
  ...
  drawProjection(base, allowTable);  if (!paid) {
...
function goToPlanner() {
  ...
  const q = new URLSearchParams({/* user profile */
...
divYield:  4  });  location.href = /calculators/planner/index.html?${q.toString()};
}function init() {
  const sellBox  = $("sellAtRet");
  const unlockBtn = $("unlockPro");  /* unlock via ?code=tepuy2025 */
  ...
  document.querySelectorAll(".pro-only")
          .forEach(el => el.style.display = paid ? "inline-block" : "none");  /* button behaviour */
  if (paid) {
...
}document.addEventListener("DOMContentLoaded", init);, what happened